...
def iniciarRed(tipo):
	...
	if tipo == True:
		auxEntradas = []
		for x in range(NEntradas):
			auxEntradas.append([0,100])

		redNeuronal = nl.net.newff(auxEntradas,[40,NSalidas])
#		redNeuronal.trainf = nl.train.train_gd
#		redNeuronal.errorf = nl.error.CEE()
	else:
		redNeuronal = nl.load("RedColores.net")
...
def cargarDatosEntrenamiento(entradas,salidas):
	...
	if(AuxEntradas.shape[0] >1):
		for aux in AuxEntradas:
			Entradas =  np.append(Entradas,aux*100)
...
iniciarRed(True)
#imagen = Image.open('C:/Users/Hugo/Desktop/U.R.L/Septimo ciclo/Inteligencia Artificial/Principal Proyecto Final/entradas/im140.jpg')
#imagen = imagen.resize((300,225))
